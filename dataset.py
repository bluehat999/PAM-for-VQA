...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    entity_path = os.path.join(dataroot, 'cache', '%s_entity_bounds1.pkl' % name)
    ...
    # test2015
        ...
        entries = []
        for question,en in zip(questions, entity):
            img_id = question['image_id']
            utils.assert_eq(question['question_id'], en['question_id'])
            if not COUNTING_ONLY \
...
class VQAFeatureDataset(Dataset):
    ...
    def tokenize(self, max_length=14, obj_max=8, rela_max=8, obj_len=6, rela_len=6):
        ...
        for entry in self.entries:
            ...
            # entry['q_onum'] = torch.tensor(num)
            # entry['q_rnum'] = torch.tensor(r_num)
    ...
    def tensorize(self, obj_max=8):
        ...
        for entry in self.entries:
            ...
            # limit entry num to tensorize question bounds and relations
    ...
    def __getitem__(self, index):
        ...
        if answer is not None:
            ...
            return features, normalized_bb, question, target,\
                ...
                semantic_adj_matrix, raw_question, q_bounds, q_objs, q_relations, q_adj
