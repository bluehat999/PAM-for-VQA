...
def parse_args():
    ...
    parser.add_argument('--gap_lamda', type=float, default=0.2)
    ...
    parser.add_argument('--saving_epoch', type=int, default=-1,
                        help='Print log for certain steps')
    parser.add_argument('--ignore_eval', action='store_true',
                        help='ignore the eval dataset when training')
    parser.add_argument('--first_batch', action='store_true',
                        help='test whether the first batch can overfitting?')
...
if __name__ == '__main__':
    ...
    ignore_eval = args.ignore_eval or args.first_batch
    ...
    model = build_model(train_dset, args).to(device)
    ...
    if args.tfidf:
        tfidf, weights = tfidf_from_questions(['train', 'val', 'test2015'],
                                              dictionary)
    model.encoderq.w_emb.init_embedding(join(args.data_folder,
    ...
    if args.use_both and args.dataset == "vqa" and not ignore_eval:
        length = len(val_dset)
        ...
        if args.use_vg:
            ...
            trainval_concat_dsets_split = random_split(
                                trainval_concat_dset,
                                [int(0*length),
                                 len(trainval_concat_dset)-int(0*length)])
        ...
        train_loader = DataLoader(final_train_dset, batch_size, shuffle=True,
                                  num_workers=4, collate_fn=trim_collate, pin_memory=True)
        if args.use_vg:
    ...
    output_meta_folder = join(args.output, "16+")
    utils.create_dir(output_meta_folder)
    args.output = output_meta_folder+"/%s_%s_%d_%s" % (
                fusion_methods,args.dataset, args.seed, args.anno)
    ...
    logger = utils.Logger(join(args.output, 'log_%s.txt'%(args.anno)))
    logger.write("Choose random seed %d" % args.seed)
    print(">>>>data and model prepared. time cost: %.2f<<<<" % ( time.time()-s))
    N1 = 0
    N2 = 0
    if args.first_batch:
        loaderit = iter(train_loader)
        first = next(loaderit)
        second = next(loaderit)
        train_loader = [first] * 1800
        eval_loader = [second] * 600
        N1 = batch_size * 1800
        N2 = batch_size * 600
    train(model, train_loader, eval_loader, args, device, N1, N2)
