...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        emb = self.emb(x)
        if 'c' in self.op:
            emb = torch.cat((emb, self.emb_(x)), -1)
...
class SentenceEncoder(nn.Module):
    def __init__(self, dim, q_dim):
        ...
        self.bilstm = nn.LSTM(input_size=dim,
                              hidden_size=dim // 2,
                              num_layers=2,
                              batch_first=True,
                              dropout=0,
                              bidirectional=True)
        self.concat = FCNet(dims=[dim *3 , q_dim], act=None, dropout=0.1)

    def forward(self, x):
        words = x.permute(0, 2, 1)
        unigrams = torch.unsqueeze(self.unigram_conv(words), 2) 
        bigrams  = torch.unsqueeze(self.bigram_conv(words), 2)  
        trigrams = torch.unsqueeze(self.trigram_conv(words), 2) 
